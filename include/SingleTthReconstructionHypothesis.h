 ...
 * SingleTthReconstructionHypothesis then consists of finding the lepton, and assigning the jets of the event
 ...
 * see SingleTthReconstructionHypothesisDiscriminators.h for different criteria and to fill the discriminators.
 */
class SingleTthReconstructionHypothesis {
public:
  ...
  float discriminator(const std::string & l) const {
      ...
      if(it == m_discriminators.end()){
          throw std::runtime_error("SingleTthReconstructionHypothesis::discriminator: discriminator with label '" + l + "' not set");
      }
      return it->second;
  }
  ...
  void set_lepton(Particle & l){m_lepton = l;}
