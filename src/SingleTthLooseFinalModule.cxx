...
namespace uhh2examples {
  ...
  class SingleTthLooseFinalModule: public ModuleBASE {
  public:

    explicit SingleTthLooseFinalModule(Context & ctx);
  ...
  void SingleTthLooseFinalModule::book_histograms(uhh2::Context& ctx, vector<string> tags){
    for(const auto & tag : tags){
      cout << "booking histograms with tag " << tag << endl;
      string mytag = "btagsf_ech_sr_" + tag;
      book_HFolder(mytag, new SingleTthHists(ctx,mytag));
      mytag = "btagsf_much_sr_" + tag;
      book_HFolder(mytag, new SingleTthHists(ctx,mytag));
      mytag = "btagsf_ech_cr_" + tag;
      book_HFolder(mytag, new SingleTthHists(ctx,mytag));
      mytag = "btagsf_much_cr_" + tag;
      book_HFolder(mytag, new SingleTthHists(ctx,mytag));
    }
  }

  void SingleTthLooseFinalModule::book_pdf_histograms(uhh2::Context& ctx, vector<string> tags){
    for(const auto & tag : tags){
      cout << "booking histograms with tag " << tag << endl;
      string mytag = "btagsf_ech_sr_" + tag;
  ...
  void SingleTthLooseFinalModule::fill_histograms(uhh2::Event& event, string tag, bool is_sr, bool is_much){
    string mytag = "btagsf_";
  ...
  SingleTthLooseFinalModule::SingleTthLooseFinalModule(Context & ctx){

    cout << "Hello World from SingleTthLooseFinalModule!" << endl;
    ...
    JetId DeepjetTight = BTag(BTag::DEEPJET, BTag::WP_LOOSE);
    ...
    if(year == Year::is2017v1 || year == Year::is2017v2){
      // 2017
      systnames = {"btag_bc", "btag_udsg"};
      handlenames = { "weight_btag" , "weight_btag"};
    ...
    systshift = {"up", "down"};
    if(systnames.size() != handlenames.size()) throw runtime_error("In SingleTthLooseFinalModule.cxx: Length of systnames and handlenames is not equal.");
  ...
  bool SingleTthLooseFinalModule::process(Event & event) {
    // cout << "++++++++++ NEW EVENT ++++++++++" << endl;
  ...
  UHH2_REGISTER_ANALYSIS_MODULE(SingleTthLooseFinalModule)
}
