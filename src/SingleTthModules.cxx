...
MuonTriggerWeights::MuonTriggerWeights(Context & ctx, TString path_, Year year_): path(path_), year(year_){
  ...
  file_30to50.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/MuonTriggerScaleFactors_eta_mu_binned_pt30to50.root","READ"));
  file_50to100.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/MuonTriggerScaleFactors_eta_mu_binned_pt50to100.root","READ"));
  file_100to200.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/MuonTriggerScaleFactors_eta_mu_binned_pt100to200.root","READ"));
  file_200toinf.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/MuonTriggerScaleFactors_eta_mu_binned_pt200toInf.root","READ"));
...
MuonTriggerWeightsOffical::MuonTriggerWeightsOffical(Context & ctx, TString path_, Year year_): path(path_), year(year_){
  ...
  bool is_mc = dataset_type == "MC";
  if(!is_mc){
    cout << "Warning: MuonTriggerWeightsOffical will not have an effect on this non-MC sample (dataset_type = '" + dataset_type + "')" << endl;
    return;
  }
  ...
  g_sf_eta1p2to2p1.reset((TGraphAsymmErrors*)file_eta1p2to2p1->Get("ScaleFactors"));
  g_sf_eta2p1to2p4.reset((TGraphAsymmErrors*)file_eta2p1to2p4->Get("ScaleFactors"));

}

bool MuonTriggerWeightsOffical::process(Event & event){
  ...
  double eta = muon.eta();
  double pt = muon.pt();
  if(fabs(eta) > 2.4) throw runtime_error("In LQToTopMuModules.cxx, MuonTriggerWeightsOffical.process(): Muon-|eta| > 2.4 is not supported at the moment.");
  if(pt < 30) throw runtime_error("In LQToTopMuModules.cxx, MuonTriggerWeightsOffical.process(): Muon-pt < 30 is not supported at the moment.");
  ...
  if(abs(eta) < 0.9){
    bool keep_going = true;
    double xmax, ymax;
    g_sf_eta0to0p9->GetPoint(g_sf_eta0to0p9->GetN()-1,xmax,ymax);
    if(pt > xmax - g_sf_eta0to0p9->GetErrorXlow(g_sf_eta0to0p9->GetN()-1)) {
      idx = g_sf_eta0to0p9->GetN()-1;
    }
    else{
      while(keep_going){
	double x,y;
	g_sf_eta0to0p9->GetPoint(idx,x,y);
	keep_going = pt > x + g_sf_eta0to0p9->GetErrorXhigh(idx);
	if(keep_going) idx++;
      }
    }
  }
    ...
    bool keep_going = true;
    double xmax, ymax;
    g_sf_eta0p9to1p2->GetPoint(g_sf_eta0p9to1p2->GetN()-1,xmax,ymax);
    if(pt > xmax - g_sf_eta0p9to1p2->GetErrorXlow(g_sf_eta0p9to1p2->GetN()-1)) idx = g_sf_eta0p9to1p2->GetN()-1;
    else{
      while(keep_going){
	double x,y;
	g_sf_eta0p9to1p2->GetPoint(idx,x,y);
	keep_going = pt > x + g_sf_eta0p9to1p2->GetErrorXhigh(idx);
	if(keep_going) idx++;
      }
    }
  }
    ...
    bool keep_going = true;
    double xmax, ymax;
    g_sf_eta1p2to2p1->GetPoint(g_sf_eta1p2to2p1->GetN()-1,xmax,ymax);
    if(pt > xmax-g_sf_eta1p2to2p1->GetErrorXlow(g_sf_eta1p2to2p1->GetN()-1)) idx = g_sf_eta1p2to2p1->GetN()-1;
    else{
      while(keep_going){
	double x,y;
	g_sf_eta1p2to2p1->GetPoint(idx,x,y);
	keep_going = pt > x + g_sf_eta1p2to2p1->GetErrorXhigh(idx);
	if(keep_going) idx++;
      }
    }
  }
    ...
    bool keep_going = true;
    double xmax, ymax;
    g_sf_eta2p1to2p4->GetPoint(g_sf_eta2p1to2p4->GetN()-1,xmax,ymax);
    if(pt > xmax - g_sf_eta2p1to2p4->GetErrorXlow(g_sf_eta2p1to2p4->GetN()-1)) idx = g_sf_eta2p1to2p4->GetN()-1;
    else{
      while(keep_going){
	double x,y;
	g_sf_eta2p1to2p4->GetPoint(idx,x,y);
	keep_going = pt > x + g_sf_eta2p1to2p4->GetErrorXhigh(idx);
	if(keep_going) idx++;
      }
    }
  }
  ...
  double stat_up = -1., stat_down = -1., tp = 0.02, total_up = -1., total_down = -1.;
  if(abs(eta) < 0.9){
    g_sf_eta0to0p9->GetPoint(idx,dummy_x,sf);

    stat_up = g_sf_eta0to0p9->GetErrorYhigh(idx);
    stat_down = g_sf_eta0to0p9->GetErrorYlow(idx);
    total_up = sqrt(pow(stat_up,2) + pow(tp,2));
    total_down = sqrt(pow(stat_down,2) + pow(tp,2));
  ...
  if(abs(eta) < 1.2){
    g_sf_eta0p9to1p2->GetPoint(idx,dummy_x,sf);

    stat_up = g_sf_eta0p9to1p2->GetErrorYhigh(idx);
    stat_down = g_sf_eta0p9to1p2->GetErrorYlow(idx);
    total_up = sqrt(pow(stat_up,2) + pow(tp,2));
    total_down = sqrt(pow(stat_down,2) + pow(tp,2));
    ...
    g_sf_eta1p2to2p1->GetPoint(idx,dummy_x,sf);

    stat_up = g_sf_eta1p2to2p1->GetErrorYhigh(idx);
    stat_down = g_sf_eta1p2to2p1->GetErrorYlow(idx);
    total_up = sqrt(pow(stat_up,2) + pow(tp,2));
    total_down = sqrt(pow(stat_down,2) + pow(tp,2));
    ...
    g_sf_eta2p1to2p4->GetPoint(idx,dummy_x,sf);

    stat_up = g_sf_eta2p1to2p4->GetErrorYhigh(idx);
    stat_down = g_sf_eta2p1to2p4->GetErrorYlow(idx);
    total_up = sqrt(pow(stat_up,2) + pow(tp,2));
    total_down = sqrt(pow(stat_down,2) + pow(tp,2));
...
// ElectronTriggerWeightsOfficial::ElectronTriggerWeightsOfficial(Context & ctx, TString path_, Year year_): path(path_), year(year_){
...
//   bool is_mc = dataset_type == "MC";
//   if(!is_mc){
//     cout << "Warning: ElectronTriggerWeightsOfficial will not have an effect on this non-MC sample (dataset_type = '" + dataset_type + "')" << endl;
//     return;
//   }
...
//   unique_ptr<TFile> file_eta0p9to1p2, file_eta0to0p9, file_eta1p2to2p1, file_eta2p1to2p4;
//   file_eta0p9to1p2.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/ElectronTriggerScaleFactors_pt_mu_binned_official_eta0p9to1p2.root","READ"));
//   file_eta0to0p9.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/ElectronTriggerScaleFactors_pt_mu_binned_official_eta0to0p9.root","READ"));
//   file_eta1p2to2p1.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/ElectronTriggerScaleFactors_pt_mu_binned_official_eta1p2to2p1.root","READ"));
//   file_eta2p1to2p4.reset(new TFile(path+"/" + yeartag + "/TriggerEfficiencies2e/ElectronTriggerScaleFactors_pt_mu_binned_official_eta2p1to2p4.root","READ"));
...
// bool ElectronTriggerWeightsOfficial::process(Event & event){
...
//   double eta = elec.eta();
//   double pt = elec.pt();
//   if(fabs(eta) > 2.4) throw runtime_error("In LQToTopMuModules.cxx, ElectronTriggerWeightsOfficial.process(): Elec-|eta| > 2.4 is not supported at the moment.");
//   if(pt < 30) throw runtime_error("In LQToTopMuModules.cxx, ElectronTriggerWeightsOfficial.process(): Elec-pt < 30 is not supported at the moment.");
