...
number_of_channels =  len(years) * len(categories)
...
#    ech_rate_unc[icat] = {}
    ...
    for year in years:
        ...
        f_much = open(folder + "SignalFitOutput_"+year+"_"+icat+"_"+ma_mass+"_"+channel+".txt", "r")
...
#    bkg_ech_norm[icat] = {}
    ...
    for year in years:
        ...
        for line in listOfLines:
            ch_name = "Muon"
            antich_name = "Electron"
            if "ech" in channel: 
                ch_name = "Electron"
                antich_name = "Muon"
            if ch_name in line and "Signal" not in line:
            ...
            if ch_name in line and "Signal" in line:
                for j in range(i+1,len(listOfLines)):
                    if antich_name in listOfLines[j]: break
...
# open file to write
for mass in masses:
    outputfile = open("Datacard_allyears_M"+str(mass)+"_"+channel+".txt",'w')
...
#    rate_sig_ech_2018 = "rate_signal_ech_2018     lnN    "
    ...
    for cat in categories:
        ...
        for year in sorted(years):
            ...
            outputfile.write("shapes * "+channel+"_"+year+"_"+cat+" ws_SingleTth_"+str(year)+"_"+cat+"_"+ma_mass+".root SingleTth"+cat+":$PROCESS_$CHANNEL \n \n")
#            outputfile.write("shapes * much_"+year+"_"+cat+" ws_SingleTth_"+str(year)+"_"+cat+"_"+ma_mass+".root SingleTth"+cat+":$PROCESS_$CHANNEL \n \n")

            bins +=  channel+"_"+year+"_"+cat+"   "
            double_bins +=  channel+"_"+year+"_"+cat+"    "+channel+"_"+year+"_"+cat+"  "
            obs +=    " -1          "
            ...
            rates += str(signal_much_norm[year+"_"+cat+"_"+mass])+"   "+str(bkg_much_norm[cat][year]) + "   "

            rate_sig += str(tot_rate_unc[cat][year][mass]) +"       -    "
            if("2016" in year):
...
#                rate_sig_ech_2017 +=  "-       -    -       -    "
    ...
    for cat in categories:
        ...
        for year in years:
            if "300" in cat and int(mass) < 800 and "2018" in year: continue
            if b_multipdf:
                outputfile.write("pdf_index_"+channel+"_"+year+"_"+cat+" discrete \n ")
...
#                    outputfile.write(rate_sig_ech_2018)
            ...
            outputfile.write("pdf_index_MT"+str(mass)+"_"+channel+"_"+year+"_"+cat+" discrete \n ")
