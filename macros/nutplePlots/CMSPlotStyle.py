...
def getStyle():
    ...
    # cmsstyle.SetTitleFont(43, "XYZ") # Set font size in pt
    # cmsstyle.SetTitleSize(18, "XYZ")
    ...
    # cmsstyle.SetLabelFont(43, "XYZ") # Set font size in pt
    # cmsstyle.SetLabelSize(16, "XYZ")
...
def draw_cmstext(position="default",b_pre = True,factor=1):
    ...
    text2.SetX(x)
    text2.SetY(y-top_margin*0.7)
    text2.SetTextFont(52)
    text2.SetTextSize(0.76*text1.GetTextSize())
    if b_pre:
        text2.Draw()
        
    return [text1,text2]

def draw_info(infotext, x, y, textsizefactor = 0.6, textalign = 31):
    ...
    text.SetY(y)
    text.SetTextFont(42)
    text.SetTextSize(textsizefactor*top_margin)
    text.Draw()

    return text

def draw_lumi(simonly =False, offset = 0.2, lumi = "41.5"):
    cmsTextSize      = 0.75
